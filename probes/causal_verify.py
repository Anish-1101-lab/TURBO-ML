...
def causal_verify_and_step_positional(target, context_ids, draft_ids, q_scalars, q_full_dists,
                                       vocab_size, temperature, generator, probe_hook, random_hook,
                                       probe_ckpt, probe_layer_index):
    """Position-restricted variant: each ablated condition is computed with
    the hook restricted to ONE verification position at a time (a separate
    forward pass per drafted position), instead of ablating every position
    in the sequence in one batched pass. This avoids cross-position
    contamination via later-layer attention when a round has k>1 drafted
    positions ablated at once.

    Consumes `generator` in EXACTLY the same call sequence/order as
    causal_verify_and_step (same torch.rand / torch.multinomial calls, same
    iteration order) so that, called with the same seeds, the clean-pass
    generation trajectory -- and therefore the resulting example set -- is
    bit-for-bit identical to the original diffuse-ablation run. Only how
    the ablated conditions are computed differs."""
    n = context_ids.shape[1]
    k = draft_ids.shape[1]
    full_ids = torch.cat([context_ids, draft_ids], dim=1)
    ...
    for i in range(k):
        ...
        probe_hook.target_positions = [pos]
        probe_hook.active = True
        with torch.no_grad():
            out_probe_abl = target(input_ids=full_ids)
        probe_hook.active = False
        ...
        random_hook.target_positions = [pos]
        random_hook.active = True
        with torch.no_grad():
            out_rand_abl = target(input_ids=full_ids)
        random_hook.active = False
